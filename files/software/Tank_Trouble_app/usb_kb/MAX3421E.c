...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	w_data[1] = val;
	alt_u8* t;
	int return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 2, w_data, 0, t, 0);
	if (return_code < 0){
		printf("error");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//read return code from SPI peripheral (see Intel documentation) 
	//if return code < 0  print an error
	alt_u8 w_data[nbytes+1];
	w_data[0] = reg+2;
	for(alt_u8 i = 0; i < nbytes; i++){
		w_data[i+1] = data[i];
	}
	alt_u8* t;
	int return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1+nbytes, w_data, 0, t, 0);
	if (return_code < 0){
		printf("error");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//if return code < 0 print an error
	alt_u8 t;
	int return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 1, &t, 0);
	if (return_code < 0){
			printf("error");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
